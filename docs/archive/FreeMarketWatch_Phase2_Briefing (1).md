...
## Part 11 — Hook Page and Smart Routing (Added June 2026)

### 11.1 Context and Purpose

The site currently routes all visitors directly to the dashboard (`/`).
This works for the already-aware. It does not work for uninitiated visitors
arriving cold from X, Substack, or word of mouth who need context before data.

The hook page addresses three reader types simultaneously:
- **The struggling:** falling behind and can't explain why
- **The comfortable:** doing fine but sensing instability
- **The curious:** intellectually engaged, want the framework

Full content in `FMW_Hook_v1.md` Version A (site page, ~220 words).

### 11.2 Route Changes

The hook becomes the homepage. The dashboard moves to `/dashboard`.
...
/contact           → Contact.tsx
/lens              → LensHub.tsx
/lens/fiat         → LensFiat.tsx
/lens/fiat/act/:n  → LearnAct.tsx
/lens/thm          → LensTHM.tsx
/lens/investing    → LensInvesting.tsx
```

**Updated routes:**
```
/                  → HookPage.tsx (new)
/dashboard         → Dashboard.tsx (moved from /)
/about             → About.tsx (unchanged)
/contact           → Contact.tsx (unchanged)
/lens              → LensHub.tsx (unchanged)
/lens/fiat         → LensFiat.tsx (unchanged)
/lens/fiat/act/:n  → LearnAct.tsx (unchanged)
/lens/thm          → LensTHM.tsx (unchanged)
/lens/investing    → LensInvesting.tsx (unchanged)
/lens/adoption     → LensAdoption.tsx (new — Part 12)
```

Update all internal links, nav references, and legacy redirects to reflect
`/dashboard` instead of `/`. Update `prerender.js` to prerender `/dashboard`
instead of `/`. Update `sitemap.xml` — add `/dashboard`, `/lens/adoption`;
keep `/` pointing to the hook page.

### 11.3 Smart Routing — Return Visitor Detection

First-time visitors land on the hook page (`/`).
Returning visitors are routed directly to `/dashboard` automatically.

**Implementation — `client/src/pages/HookPage.tsx`:**
...
export function HookPage() {
  ...
  useEffect(() => {
    const hasVisited = localStorage.getItem('fmw_visited');
    if (hasVisited) {
      navigate('/dashboard', { replace: true });
    }
  }, []);

  return <HookPageContent onCTAClick={handleCTAClick} />;
}

function handleCTAClick(destination: '/lens' | '/dashboard') {
  localStorage.setItem('fmw_visited', 'true');
  navigate(destination);
}
```

**Flag behavior:**
- Set only on CTA click — not on passive landing (no engagement = no flag)
- New device or cleared browser → user sees hook again (correct behavior)
- Explicit reset: small "start over" link in Footer clears the key:
  `localStorage.removeItem('fmw_visited')`

**Deep link behavior:**
Direct links to any route other than `/` bypass the hook entirely,
regardless of flag. No change needed — React Router handles this.

**SSR / prerendering note:**
The hook page at `/` should be prerendered. Add `/` back to `prerender.js`
routes list. The `useEffect` localStorage check runs only client-side after
hydration — the prerendered HTML will show the hook content, which is correct.

### 11.4 Hook Page Content

Implement from `FMW_Hook_v1.md` Version A faithfully. Key structure:

**Opening (all three reader types):**
...
**Body:** wage/housing observations → blame instinct redirected → structure
not people → mechanism named without explanation → well-intentioned person
argument → structure is the problem

**Closing:**
> "The tools to replace it exist. What's missing is broad understanding —
> and without it, we remain trapped in a system of our own making."

**Two CTAs (equal weight, side by side desktop / stacked mobile):**
- Primary: `[The Lens →]` → navigates to `/lens`, sets flag
- Secondary: `[The Dashboard →]` → navigates to `/dashboard`, sets flag

**Small fallback line below CTAs:**
> "Already familiar? [Go to Dashboard →]"
> (same behavior as secondary CTA — for users with cleared localStorage)

**Design:**
- Max width 680px centered — reading page, not data page
- Syne for opening line (larger, more weight)
- Body text: readable 16px, line-height 1.75, `var(--text-secondary)` minimum
- No charts, no data, no jargon
- Same dark background as rest of site
- No nav clutter — keep the page clean; NavBar present but minimal

### 11.5 Nav and Footer Updates

**NavBar.tsx** — update links:
- Logo/wordmark: links to `/` (hook — always)
- "Dashboard" nav item: links to `/dashboard`
- "The Lens" dropdown: unchanged (sub-items already correct)
- "About" and "Contact": unchanged

**NavBar THE LENS dropdown** — add fourth item:
```
The Lens ▾
  ├── Why Fiat Distorts    → /lens/fiat
  ├── The THM Lens         → /lens/thm
  ├── Investing with THM   → /lens/investing
  └── Bitcoin Adoption ✦  → /lens/adoption   ← new, with "In Development" indicator
```

**Footer.tsx** — update Dashboard link to `/dashboard`.
Add "Start Over" link that calls `localStorage.removeItem('fmw_visited')`.

### 11.6 FMW_Content.md Updates Needed

When this is implemented, update `FMW_Content.md`:
- Change `/` row from Dashboard to HookPage
- Add `/dashboard` row as Dashboard (moved)
- Add `/lens/adoption` row as LensAdoption (new, in development)
- Update prerendering table to reflect `/` as prerendered hook, `/dashboard` as not prerendered
- Update sitemap note

---

## Part 12 — Bitcoin Adoption Index Placeholder (Added June 2026)

### 12.1 Purpose and Philosophy

The Bitcoin Adoption Index is a stated development goal. It measures
Bitcoin's progress toward functioning as a medium of exchange — not its
price, not sentiment, but actual use as money.

This is intellectually distinct from existing Bitcoin metrics (Fear &
Greed, MVRV, NVT) which are price-sentiment tools. The adoption index
tracks functional monetary adoption: is Bitcoin actually becoming money?

The index connects directly to the THM framework: THM represents the
destination (Bitcoin as mature monetary standard). The adoption index
tracks how far along that path the world actually is. Together they give
a picture neither price charts nor on-chain analytics alone can provide.

**Methodology is under active development. Do not build the index yet.**
Build only the placeholder page that signals the intent.

### 12.2 New File

Create `client/src/pages/LensAdoption.tsx`.
...
Status badge: [In Development] — use the same badge style as
              existing "In development" items in LensHub.tsx
...
Section 2 — Why it matters (two paragraphs):
Para 1: "THM represents where Bitcoin ends up as a mature monetary
...
Para 2: "The index focuses on Medium of Exchange (MOE) as the primary
...
Section 3 — Notify me (simple form):
Heading: "Get notified when the adoption index launches"
[Email input field] [Notify Me button]
Use same Formspree endpoint as contact form, with hidden field:
subject = "Adoption Index Notification"
On success: "You're on the list."

Section 4 — Back navigation:
Link: ← Back to The Lens
```

### 12.4 LensHub.tsx Update

Update the Lens hub to show four components. The fourth card:

```
Title: Bitcoin Adoption Index
Status: In Development
Description: "Tracking Bitcoin's transition from asset to monetary
standard — measuring actual use as a medium of exchange against
global payment benchmarks."
Link: → /lens/adoption
Visual treatment: Same card layout, muted border (rgba(255,255,255,0.04)
instead of default), "In Development" badge in amber/yellow
(not THM green — green is for live content).
```

**Badge colors by status (establish as convention):**
- Live / Established: THM green `#a8ff78`
- Open questions: amber `#fbbf24`
- In Development: slate `#94a3b8`

### 12.5 Prerendering and SEO

Add `/lens/adoption` to `prerender.js` routes list.
Add to `sitemap.xml`.
Page title: `"Bitcoin Adoption Index | Free Market Watch"`
Meta description: `"Tracking Bitcoin's adoption as a medium of exchange.
An index measuring real monetary use — Lightning Network volume, merchant
acceptance, and payment flows — normalized against global benchmarks."`

### 12.6 What Not To Build Yet

- No index calculation, formula, or data display
- No chart or visualization
- No API endpoint for adoption data
- No commitment to specific data sources or weighting methodology
- No timeline promises

---

*Phase 2 briefing — additions June 2026*
*Hook page content: FMW_Hook_v1.md Version A*
*Adoption index methodology: ongoing development — see project conversations*
